...
**اختبار 7: `Custom Words: Board Integrity`**
```
يتحقق من:
  ✓ المزامنة المدمجة: تم تمرير 25 كلمة مخصصة
  ✓ اكتمال القاموس: دمج كلمات أقل من 25 مع القاموس الافتراضي بنجاح (Padding)
  ✓ أمان الذاكرة: اقتصاص الكلمات الزائدة عن 25 لتجنب كسر اللوحة (Trimming)

فشله يعني: دالة _generateInitialState في gameProvider.dart تعاني من مشكلة في دمج أو اقتصاص List<String>.
```
...
فشله يعني (بالأولوية):
  1. لا يوجد إنترنت
  2. التطبيق غير مُسجّل في Appwrite (خطأ `Invalid Origin` - يجب إضافة `com.example.p2p_codenames` كمنصة Android في لوحة Appwrite)
  3. خادم Appwrite بطيء أو تجاوزت الحد المجاني
  4. المشروع محذوف أو معطل في لوحة Appwrite
  5. Project ID أو Endpoint خاطئ في appwrite_providers.dart
```
...
### سيناريو 5: فشل أو تأخر تهيئة قاعدة بيانات Hive عند التشغيل

**ماذا يحدث؟**
```
- Hive يستهلك أكثر من 4 ثوانٍ في التهيئة ويُلغى لتفادي الـ ANR.
- المستودع المحلي `settingsBox` لا يُفتح.
- الكود في `start_screen.dart` لا يستطيع قراءة اسم المستخدم المحفوظ.
```

**الحل المبرمج (الجديد):**
لمنع المستخدم من التعطل بسبب حقل ادخال فارغ وتنبيه "الرجاء إدخال اسمك أولاً" الصامت، يقوم الكود تلقائياً باكتشاف عدم فتح Hive ويضع القيمة `"العميل"` كاسم افتراضي، مما يسمح للمستخدم بالعبور بنجاح لـ `RoomSettingsScreen`. وفي نفس الوقت، `room_settings_screen.dart` مُحصَّنة لتفادي الكراش عند محاولة قراءة الاسم باستخدام `Hive.isBoxOpen` كشرط مسبق.
